...
const BOOKING_API = process.env.BOOKING_API_URL || 'http://69.62.126.13:4000';
...
export async function GET(
  ...
  const url = `${BOOKING_API}${apiPath}${queryString}`;
  
  console.log('[Booking Proxy GET]', url);
...
export async function POST(
  ...
  // Ensure path starts with /api/ if not already present
  const apiPath = path.startsWith('api/') ? `/${path}` : `/api/${path}`;
  const url = `${BOOKING_API}${apiPath}`;
  
  console.log('[Booking Proxy POST]', url);
